...
## Conda-forge platforms (what ships today, how to add more)

Packages on [anaconda.org/conda-forge/vbmicrolensing](https://anaconda.org/conda-forge/vbmicrolensing) are **subdir-specific**. As of early 2026 the feedstock only generates CI for:

| Subdir        | Typical use              | Shipped for `vbmicrolensing` |
|---------------|--------------------------|------------------------------|
| `linux-64`    | Linux x86_64             | Yes                          |
| `osx-64`      | Intel macOS              | Yes                          |
| `win-64`      | Windows x86_64           | Yes                          |
| `osx-arm64`   | Apple Silicon (arm64)    | **No** (common `conda install` failure on M1/M2/M3) |
| `linux-aarch64` | Linux arm64 (e.g. AWS Graviton, many SBCs) | **No** |
| `win-arm64`   | Windows on ARM           | **No**                       |
| `linux-ppc64le` | POWER                    | **No**                     |

PyPI wheels can still cover some of those (for example `osx-arm64`); conda-forge only lists builds that the **feedstock** actually produces.

### Enabling `osx-arm64` (and optionally `linux-aarch64`)

This is done in **[vbmicrolensing-feedstock](https://github.com/conda-forge/vbmicrolensing-feedstock)**, not in this source repo: edit `conda-forge.yml`, then re-render (bot or local `conda-smithy`) so `.ci_support` and `.github/workflows/conda-build.yml` gain the new matrix rows.

Minimal change for **Apple Silicon** conda packages (cross-compile arm64 artifacts from the existing Intel macOS job, then test per conda-forge defaults):
...
Merge those keys with the existing top-level keys in the feedstock’s `conda-forge.yml` (keep `github`, `conda_build`, `conda_forge_output_validation`, etc.).

To also ship **Linux aarch64** wheels from the existing `linux-64` CI image (common conda-forge pattern):
...
Then open a PR from a **fork**, and either:

- comment `@conda-forge-admin, please rerender` on the PR so the bot regenerates workflows, or
- run `conda smithy rerender` locally and commit the regenerated files.

After merge, wait for the feedstock pipeline to build and for the CDN to sync (often on the order of an hour or two). See also the conda-forge [knowledge base](https://conda-forge.org/docs/maintainer/knowledge_base/) (Apple Silicon / cross-compilation).
